            ...
            <div class="row ">
                <div class="col-md-7 comp-grid">
                    ...
                    <div  class="bg-light p-3 animated fadeIn page-content">
                        <form id="roles_courrier-add-form" role="form" novalidate enctype="multipart/form-data" class="form page-form form-horizontal needs-validation" action="<?php print_link("roles_courrier/add?csrf_token=$csrf_token") ?>" method="post">
                            <div>
                                        ...
                                        <div class="col-sm-8">
                                            <div class="">
                                                <select required=""  id="ctrl-roles" name="roles"  placeholder="<?php print_lang('s_lectionnez_une_valeur'); ?>"    class="custom-select" >
                                                    ...
                                                    <?php 
                                                    $roles_options = $comp_model -> roles_courrier_roles_option_list();
                                        ...
                                        <div class="col-sm-8">
                                            <div class="">
                                                <select required=""  id="ctrl-etat" name="etat"  placeholder="<?php print_lang('s_lectionnez_une_valeur'); ?>"    class="custom-select" >
                                                    <option value=""><?php print_lang('s_lectionnez_une_valeur'); ?></option>
                                                    <?php 
                                                    $etat_options = $comp_model -> roles_courrier_etat_option_list();
                                                    if(!empty($etat_options)){
                                                    foreach($etat_options as $option){
                                                    $value = (!empty($option['value']) ? $option['value'] : null);
                                                    $label = (!empty($option['label']) ? $option['label'] : $value);
                                                    $selected = $this->set_field_selected('etat',$value, "");
                                                    ?>
                                                    <option <?php echo $selected; ?> value="<?php echo $value; ?>">
                                                        <?php echo $label; ?>
                                                    </option>
