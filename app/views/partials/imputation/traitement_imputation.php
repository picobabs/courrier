...
<section class="page" id="<?php echo $page_element_id; ?>" data-page-type="list"  data-display-type="grid" data-page-url="<?php print_link($current_page); ?>">
    ...
    <div  class="bg-light p-3 mb-3">
        <div class="container-fluid">
            <div class="row ">
                <div class="col ">
                    <h5 class="record-title"><?php print_lang('traitement_des_imputations'); ?></h5>
                </div>
                <div class="col-sm-3 ">
                    <?php if($can_add){ ?>
            ...
            <div class="row ">
                <div class="col-sm-3 comp-grid">
                    <form method="get" action="<?php print_link($current_page) ?>" class="form filter-form">
                        <div class="card mb-3">
                            <div class="card-header h4 h4"><h5>Etat Traitement</h5></div>
                            <div class="p-2">
                                <select   name="imputation_etat_traitement" class="form-control custom ">
                                    <option value=""><?php print_lang('s_lectionnez_une_valeur'); ?></option>
                                    <?php 
                                    $imputation_etat_traitement_options = $comp_model -> imputation_imputationetat_traitement_option_list();
                                    if(!empty($imputation_etat_traitement_options)){
                                    foreach($imputation_etat_traitement_options as $option){
                                    $value = (!empty($option['value']) ? $option['value'] : null);
                                    $label = (!empty($option['label']) ? $option['label'] : $value);
                                    $selected = $this->set_field_selected('imputation_etat_traitement',$value);
                                    ?>
                ...
                <div class="col-sm-9 comp-grid">
                    ...
                    <div class="filter-tags mb-2">
                        <?php
                        if(!empty(get_value('imputation_etat_traitement'))){
                        ?>
                        <div class="filter-chip card bg-light">
                            <b>Imputation Etat Traitement :</b> 
                            <?php 
                            if(get_value('imputation_etat_traitementlabel')){
                            echo get_value('imputation_etat_traitementlabel');
                            }
                            else{
                            echo get_value('imputation_etat_traitement');
                            }
                            $remove_link = unset_get_value('imputation_etat_traitement', $this->route->page_url);
                            ?>
                    ...
                    <div  class=" animated fadeIn page-content">
                        <div id="imputation-traitement_imputation-records">
                            <?php
                            if(!empty($records)){
                            ?>
                                    ...
                                    <div class="col-sm-6">
                                        <div class="arrondic p-2 mb-3 animated bounceIn">
                                            <div class="td-btn">
                                                <?php if($can_edit){ ?>
                                                <?php
                                                // Imputer a son tour le courrier a un agent de l'echelon inferieur.
                                                // Un directeur qui recoit un courrier peut ainsi le transmettre a un
                                                // chef de division sans quitter cet ecran. Le courrier est passe en
                                                // parametre pour pre-remplir le formulaire et restreindre la liste
                                                // des destinataires proposes.
                                                $id_courrier_ligne = $data['idcourrier'] ?? null;
                                                if (!empty($id_courrier_ligne)):
                                                ?>
                                                <a class="btn btn-sm btn-success has-tooltip imputer_agent" title="Imputer ce courrier a un agent" href="<?php print_link("imputation/add/?idcourrier=" . urlencode($id_courrier_ligne)); ?>">
                                                    <i class="material-icons">person_add</i>
                                                </a>
                                                <?php endif; ?>
                                                <a class="btn btn-sm btn-info has-tooltip" title="<?php print_lang('modifier_cet_enregistrement'); ?>" href="<?php print_link("imputation/edit_traitement/$rec_id"); ?>">
