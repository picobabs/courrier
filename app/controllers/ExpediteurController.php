<?php 
/**
 * Expediteur Page Controller
 * @category  Controller
 */
class ExpediteurController extends SecureController{
	function __construct(){
		parent::__construct();
		$this->tablename = "expediteur";
	}
	/**
     * List page records
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$this->view->search_template = "expediteur/search.php";
		}
		...
		$page_title = $this->view->page_title = get_lang('expediteur');
		$this->render_view("expediteur/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("expediteur.idexpediteur", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = get_lang('vue_expediteur');
		...
		return $this->render_view("expediteur/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("expediteur","cellulaire");
			...
			if($this->validated()){
				...
				foreach (array('cellulaire' => '', 'ordinaire' => 0) as $col => $defaut) {
				...
				if($rec_id){
					...
					return	$this->redirect("expediteur");
				}
				else{
					$this->set_page_error();
				}
		...
		$this->render_view("expediteur/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("idexpediteur","expediteur","cellulaire");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'expediteur' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("expediteur.idexpediteur", $rec_id);;
		...
		return $this->render_view("expediteur/edit.php", $data);
	}
	/**
     * Update single field
	 * @param $rec_id (select record by table primary key)
	...
	function delete($rec_id = null){
		...
		$db->where("expediteur.idexpediteur", $arr_rec_id, "in");
