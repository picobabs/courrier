...
class DirectionController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$this->view->search_template = "direction/search.php";
		}
		if(!empty($request->orderby)){
			...
			$db->orderBy("direction.iddirection", ORDER_TYPE);
		}
		...
		$page_title = $this->view->page_title = get_lang('direction');
		$this->render_view("direction/list.php", $data); //render the full page
	}
	/**
     * View record detail 
	...
	function view($rec_id = null, $value = null){
		...
		if($value && est_nom_de_colonne($rec_id)){
			...
			$db->where("direction.iddirection", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = get_lang('vue_direction');
		...
		return $this->render_view("direction/view.php", $record);
	}
	/**
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("direction");
		...
		$this->render_view("direction/add.php");
	}
	/**
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("iddirection","direction");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'direction' => 'required',
			...
			$modeldata = $this->modeldata = $this->validate_form($postdata);
			if($this->validated()){
				$db->where("direction.iddirection", $rec_id);;
		...
		return $this->render_view("direction/edit.php", $data);
	}
	/**
     * Update single field
	 * @param $rec_id (select record by table primary key)
	...
	function delete($rec_id = null){
		...
		$db->where("direction.iddirection", $arr_rec_id, "in");
