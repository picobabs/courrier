...
class BaseController{
	...
	function __construct(){
		...
		$this->file_upload_settings['fichier'] = array(
			"title" => "{{random}}",
		...
		$this->file_upload_settings['ftraite'] = array(
			"title" => "{{random}}",
			"extensions" => ".docx,.doc,.xls,.xlsx,.xml,.csv,.pdf,.xps,.jpg,.jpeg,.png",
			"limit" => "1",
			"maxSize" => "30",
			"returnfullpath" => true,
			"filenameprefix" => "",
		...
		// Les deux blocs qui figuraient ici redefinissaient 'ftraite' et 'fichier'
		// avec "extensions" => "" (aucun controle de type) et, pour 'fichier',
		// le nom d'origine conserve. Comme ils ecrasaient les definitions
		// correctes ci-dessus, n'importe quel fichier - y compris un .php -
		// pouvait etre depose dans uploads/files/ puis execute. Blocs supprimes.
	...
	function get_pagination($page_count = MAX_RECORD_COUNT){
		...
		// limit_count et limit_start proviennent de l'URL. Sans controle, une
		// valeur non numerique provoquait une erreur fatale en PHP 8, et une
		// valeur enorme permettait de vider toute la table en une requete.
		$limit_count = (!empty($request->limit_count) ? (int) $request->limit_count : (int) $page_count);
		$limit_start = (!empty($request->limit_start) ? (int) $request->limit_start : 1);
		if ($limit_count < 1 || $limit_count > 200) { $limit_count = (int) $page_count; }
