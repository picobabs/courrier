...
class Router
{
	...
	function run($url)
	{
		...
		$page = safe_sanitize_string($page);
		$action = safe_sanitize_string($action);
		$fieldname = safe_sanitize_string($fieldname);
		$fieldvalue = safe_sanitize_string($fieldvalue);
		...
		$args = safe_sanitize_string($args);
