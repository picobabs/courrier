 ...
 */
/**
 * Les proprietes de cette classe sont creees dynamiquement dans le
 * constructeur a partir des parametres de l'URL ($_GET), motif qui
 * declenche l'avertissement "Creation of dynamic property" depuis PHP 8.2.
 * L'attribut ci-dessous autorise explicitement ce fonctionnement.
 * (Sur PHP 7, cette ligne est ignoree : elle y est lue comme un commentaire.)
 */
...
class BaseView
{
	...
	*/
	...
	/**
	 * Arguments passed to a partial view via render_view().
	 * Declared explicitly because creating it dynamically is deprecated
	 * since PHP 8.2 and printed a "Creation of dynamic property" notice
	 * at the top of every page.
	 * @var mixed
	 */
	public $view_args = null;
	...
	function __construct($arg = null)
	{
		...
		$get = $this->form_data =  safe_sanitize_string($_GET);;
		if (!empty($get)) {
	...
	public function render($view_name = null, $view_data = null, $layout = "main_layout.php")
	{
		...
		elseif ($page_format == "word") {
			...
			// Un nom d'onglet vide produit un fichier Excel que Excel refuse d'ouvrir.
			$sheet_name = !empty($this->report_title) ? $this->report_title : 'Export';
			//excel headers
			...
			// current() renvoie false sur un tableau vide : array_keys(false) est
			// une erreur fatale en PHP 8. On sort proprement si aucun enregistrement.
			$first_row = !empty($records) ? current($records) : null;
			if (empty($first_row) || !is_array($first_row)) {
				exit;
			}
			$arr_titles = array_keys($first_row);// get the table header from the record.
	...
	{
		$post =  safe_sanitize_string($_REQUEST);
...
<?php
	...
	{
		ob_start();
		// basename() empeche qu'un nom de gabarit contenant "../" fasse inclure
		// un fichier arbitraire du serveur.
		include(LAYOUTS_DIR . basename((string) $this->report_layout)); //render page content as html into a variable
		...
		// Load DOM
		// libxml_disable_entity_loader() est obsolete depuis PHP 8.0 (les entites
		// externes y sont desactivees par defaut). L'appeler declenche un avertissement
		// que Dompdf transforme en exception fatale : l'export PDF plantait.
		$legacyLibxml = PHP_VERSION_ID < 80000;
		if ($legacyLibxml) {
