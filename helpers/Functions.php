...
/**
 * Remplacant de filter_var($v, FILTER_SANITIZE_STRING), constante depreciee
 * depuis PHP 8.1 (elle emettait un avis a chaque appel, ce qui polluait
 * l'affichage et invalidait les reponses AJAX/JSON).
 *
 * IMPORTANT : cette fonction doit reproduire EXACTEMENT l'ancien comportement,
 * a savoir supprimer les balises puis encoder UNIQUEMENT les guillemets
 * simples et doubles. Il ne faut surtout pas utiliser htmlspecialchars() ici :
 * celui-ci encode aussi le "&", si bien qu'une valeur deja enregistree serait
 * re-encodee a chaque modification ("R&D" -> "R&amp;D" -> "R&amp;amp;D"),
 * corrompant peu a peu les libelles en base. La version ci-dessous est
 * idempotente : reappliquee sur son propre resultat, elle ne le change plus.
 *
 * Fonctionne sur les scalaires, null et les tableaux (y compris imbriques).
 * @param mixed $value
 * @return mixed
 */
function safe_sanitize_string($value)
{
	...
	if (is_array($value)) {
		return array_map('safe_sanitize_string', $value);
	}
	if (!is_scalar($value)) {
		return $value;
	}
 ...
 * de droits (droit_select, droit_lister, roles_courrier). Quand ces tables ne
 * contiennent aucune ligne pour un profil, les listes reviennent vides et
 * l'application devient inutilisable pour lui : un champ obligatoire sans
 * option ne peut jamais etre renseigne. Un administrateur ne doit pas dependre
 * de ce parametrage.
 * @return bool
 */
function utilisateur_est_administrateur()
{
	if (!defined('ROLES_ADMINISTRATEUR') || empty(ACL::$user_role)) {
		return false;
	}
	$normaliser = function ($v) {
		$v = trim((string) $v);
		$v = function_exists('mb_strtolower') ? mb_strtolower($v, 'UTF-8') : strtolower($v);
		return strtr($v, array('à'=>'a','â'=>'a','é'=>'e','è'=>'e','ê'=>'e','ë'=>'e','î'=>'i','ï'=>'i','ô'=>'o','ù'=>'u','û'=>'u','ç'=>'c'));
	};
	$admins = array_map($normaliser, explode(',', ROLES_ADMINISTRATEUR));
	return in_array($normaliser(ACL::$user_role), $admins);
}

/**
 * Indique si l'utilisateur connecte a vocation a voir les imputations de tous
 * les agents, et non seulement celles qui lui sont adressees.
 *
 * C'est le cas des administrateurs et des profils du sommet de l'organigramme
 * (ROLES_ACCES_INTEGRAL), ainsi que des profils de vue transversale
 * (ROLES_VUE_TRANSVERSALE) : le Secretaire General doit suivre l'ensemble du
 * circuit. Pour tous les autres, un ecran d'imputations n'a de sens que s'il
 * montre les leurs.
 * @return bool
 */
function utilisateur_voit_toutes_les_imputations()
{
	if (empty(ACL::$user_role)) {
		return false;
	}
	$normaliser = function ($v) {
		$v = trim((string) $v);
	...
	$superviseurs = array();
	foreach (array('ROLES_ADMINISTRATEUR', 'ROLES_ACCES_INTEGRAL', 'ROLES_VUE_TRANSVERSALE') as $constante) {
		if (defined($constante) && constant($constante) !== '') {
			$superviseurs = array_merge($superviseurs, array_map($normaliser, explode(',', constant($constante))));
		}
	}
	return in_array($normaliser(ACL::$user_role), $superviseurs, true);
}

/**
 * Indique si l'utilisateur connecte a vocation a voir l'ensemble du courrier,
 * et non seulement celui qui lui est impute.
 *
 * Deux familles de profils : la supervision, qui suit tout le circuit, et le
 * bureau du courrier, qui enregistre les plis avant qu'ils ne soient imputes
 * a qui que ce soit. Pour tous les autres, un courrier ne les concerne que
 * s'il leur a ete impute.
 * @return bool
 */
function utilisateur_voit_tous_les_courriers()
{
	if (function_exists('utilisateur_voit_toutes_les_imputations') && utilisateur_voit_toutes_les_imputations()) {
		return true;
	}
	if (!defined('ROLES_VUE_COMPLETE_COURRIER') || empty(ACL::$user_role)) {
		return false;
	}
	$normaliser = function ($v) {
		$v = trim((string) $v);
	...
	$exempts = array_map($normaliser, explode(',', ROLES_VUE_COMPLETE_COURRIER));
	return in_array($normaliser(ACL::$user_role), $exempts, true);
}

/**
 * Condition SQL restreignant une requete sur la table courrier aux seuls
 * courriers imputes a l'utilisateur connecte. Renvoie null lorsque
 * l'utilisateur a vocation a tout voir, auquel cas aucune restriction ne doit
 * etre appliquee.
 * @return string|null
 */
function condition_courriers_imputes()
{
	if (utilisateur_voit_tous_les_courriers()) {
		return null;
	}
	return " EXISTS (SELECT 1 FROM imputation im WHERE im.idcourrier = courrier.idcourrier AND im.iduser = '" . intval(USER_ID) . "') ";
}
...
function arr_to_csv($data)
{
	...
	// Sur un tableau vide, current() renvoie false et array_keys(false) provoque
	// une erreur fatale en PHP 8 : on retourne une chaine vide dans ce cas.
...
// NOTE: str_contains() is a native PHP function since PHP 8.0.
// The custom polyfill that used to live here has been removed because
// redeclaring a built-in function is a fatal error on PHP 8+ and was
// crashing the whole application on every single page load.
// (It was not used anywhere else in the codebase.)
/**
...
{
	$post =  safe_sanitize_string($_POST);
	if (!empty($post[$field])) {
		return $post[$field];
	} else {
		return $default_value;
